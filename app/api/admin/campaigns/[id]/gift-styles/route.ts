...
// POST /api/admin/campaigns/:id/gift-styles — 新增款式登記（名稱＋門檻金額；也支援圖片，跟一般商品一致）
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!isFinite(thresholdAmount) || thresholdAmount <= 0) return NextResponse.json({ error: "門檻金額格式不正確" }, { status: 400 });
