/**
 * 自製的簡易試算表公式引擎（不依賴 Handsontable / HyperFormula，避免商業授權問題，見3.3節）
 * 支援：基本四則運算 + - * /、括號、儲存格參照（如A1）、SUM(A1:A5)範圍加總
 */
...
function evaluateCell(data: SheetData, row: number, col: number, cache: Map<string, number>, visiting: Set<string>): number {
  const key = `${row},${col}`;
...
function evaluateExpression(expr: string, data: SheetData, cache: Map<string, number>, visiting: Set<string>): number {
  ...
  if (!/^[\d\s+\-*/().]*$/.test(expr)) throw new Error("公式包含不支援的字元");
